...
namespace Eigen {
  ...
  */
  ...
  //9.1.16 default constructor
  EigenMultivariateNormal()
  {
	  MatrixXf defMean(1, 1);
	  defMean << 1.0;
	  MatrixXf defCov(1, 1);
	  defCov << 1.0;
	  const uint64_t defSeed = std::mt19937::default_seed;
	  _use_cholesky = false;
	  setMean(defMean);
	  setSeed(defSeed);
	  setCovar(defCov);
	...
	void setSeed(const uint64_t &seed ) { randN.seed(seed); }
    void setCovar(const Matrix<Scalar,Dynamic,Dynamic>& covar, const bool use_cholesky = false)
    {
		  _covar = covar;
		  _use_cholesky = use_cholesky;
		  ...
		  if (_use_cholesky)
		{
		  Eigen::LLT<Eigen::Ref<Eigen::Matrix<Scalar,Dynamic,Dynamic>> > cholSolver(_covar);
			  ...
			  if (cholSolver.info()==Eigen::Success)
				{
				  ...
				  //_covar = _covar.triangularView<Lower>();
				}
			  else
				{
				  //11.12.21--throw std::runtime_error("Failed computing the Cholesky decomposition. Use solver instead");
    ...
    Matrix<Scalar,Dynamic,-1> samples(int nn)
      {
		/*if (_use_cholesky) {
			return (_covar * Matrix<Scalar, Dynamic, -1>::NullaryExpr(_covar.rows(), 
				                                        nn, randN)).colwise() + _mean;
		}
		else {*/
		return (_transform * Matrix<Scalar, Dynamic, -1>::NullaryExpr(_covar.rows(), 
			                                           nn, randN)).colwise() + _mean;
		//}
